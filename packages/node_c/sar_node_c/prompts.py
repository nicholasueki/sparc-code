...
from sar_common.types import Action, OptionMeta, OptionSet, ThinkRequest

log = logging.getLogger("sar.prompts")

THINK_TASK = """\
TASK: You are deciding SAR's next move. Propose {max_options} DISTINCT candidate \
actions from this exact set: {action_set}. Options must \
differ in kind or intent, not wording. Then choose the best one for SAR's personality \
...
enroll_face = {{"name": ...}} — ONLY when someone SAR doesn't recognize has just told \
...
def build_think_user(req: ThinkRequest) -> str:
    ...
    if req.memory:
        parts.append(
            "MEMORY (the COMPLETE list of what SAR knows from the past — if an "
            f"answer is not here or in CONVERSATION, SAR does NOT know it and says so): {req.memory}"
        )
    else:
        parts.append(
            "MEMORY: (empty — SAR has no stored knowledge; he must not claim to "
            "remember anything)"
        )
    if req.conversation:
