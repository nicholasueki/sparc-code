...
the designed path) -> sar/audio/transcript on the bus -> orchestrator thinks
-> sar/tts/say -> spoken through the Mac with `say`.

Echo control is half-duplex: the mic is gated while SAR speaks (+ a tail),
...
Run: python -m sar_node_c.earsd  (needs macOS mic permission the first time)
"""
...
from sar_common.types import SpeakRequest, Transcript

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(name)s %(message)s")
log = logging.getLogger("sar.earsd")

# Wake-name matcher for the interim address gate. Whitespace/dot/hyphen tolerant so
# STT spellings "SAR", "S.A.R.", "S A R", "S-A-R" all count as being addressed;
# \b anchors keep it from firing inside "necessary", "Caesar", or "Sarah".
NAME_RE = re.compile(r"\bs[.\s-]?a[.\s-]?r\b", re.IGNORECASE)
...
def speaker_loop() -> None:
    """Subscribe to sar/tts/say; speak via macOS `say`; gate the mic meanwhile."""
    ...
    client.subscribe("sar/tts/say", qos=1)
...
def publish_transcript(text: str, conf: float) -> None:
    ...
    mqtt_publish.single(
        "sar/audio/transcript", tr.model_dump_json(), qos=1,
...
def mic_loop() -> None:
    ...
    def flush():
        ...
        # SAR — otherwise he answers the TV, music, and passing conversation.
        # Word-boundary match: a bare `"sar" in text` substring test would fire on
        # "necessary", "Caesar", "Sarah". Accepts SAR / S.A.R. / S A R / S-A-R.
        if CFG.get("require_name", True) and not NAME_RE.search(text):
            log.info("dropped (not addressed to SAR): %.60r", text)
            return
