...
Run: python -m sar_node_b.genaid
"""
...
from sar_common.types import SalienceDecision, SalienceRequest, SelectorRequest, Selection

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(name)s %(message)s")
log = logging.getLogger("sar.genaid")

CFG = config.get("node_b", {})
GROUP_ID = "SHARED"  # hailo-apps SHARED_VDEVICE_GROUP_ID — do not change

app = FastAPI(title="sar-genaid")
...
def select(req: SelectorRequest) -> Selection:
    ...
    messages = [
        _msg("system",
             f"You are SAR's instinct. Persona: {req.persona_line}. Mood: {req.mood}. "
             "Pick the option that fits SAR best. Answer with ONE letter only."),
        _msg("user", f"{menu}\nAnswer:"),
    ]
    t0 = time.time()
