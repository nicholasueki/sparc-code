...
Run: python -m sparc_node_a.orchestrator  (mosquitto must be running locally)
"""
...
log = logging.getLogger("sparc.orchestrator")
...
class Orchestrator:
        ...
        for det in frame.detections:
            ...
            if frame.scene_delta == "new_track":
                ...
                if eid:
                    ...
                    self.world.add_event("person_left", "they left SPARC's view", [eid], 0.3)
    ...
    def _drain_births(self) -> None:
        ...
        for eid, deadline in list(self._pending_births.items()):
            ...
            d = Deliberation(
                ...
                + ("" if name else " (SPARC doesn't recognize them)"),
                tier=Tier.COMPETE,
    ...
    def on_sound(self, snd: SoundEvent) -> None:
        ...
        if prio >= 0.6:
            self.submit(Deliberation(
                ...
                trigger_desc=f"SPARC heard what sounded like {snd.cls}",
    ...
    def run(self) -> None:
        self.bus.subscribe("sparc/vision/tier0", DetectionFrame, self.on_tier0)
        self.bus.subscribe("sparc/vision/rich", DetectionFrame, self.on_rich)
    ...
    def execute(
        ...
        *,
        ...
        if action.kind in ("say", "ask_user"):
            ...
            self.bus.publish("sparc/tts/say", SpeakRequest(text=text))
            self.world.conversation.append({"role": "sparc", "text": text, "ts": time.time()})
            self.world.add_event("sparc_said", f'SPARC said: "{text}"', d.entity_ids, 0.4)
            log.info("SPARC SAYS: %s", text)
        elif action.kind == "remember":
            stmt = action.args.get("statement", "")
            self._commit_fact(stmt, source="observed")
            log.info("SPARC REMEMBERS: %s", stmt)
            # a remember with no reply feels like being ignored — brief deterministic ack
            ack = "Got it — I'll remember that."
            self.bus.publish("sparc/tts/say", SpeakRequest(text=ack))
            ...
            if eid:
                self.world.add_event("enrolled", f"SPARC learned {name}'s face",
                                     [eid], 0.7)
                self._commit_fact(f"{name} is someone SPARC knows by sight",
                                  source="observed", confidence=0.8)
                ack = f"Nice to meet you, {name} — I'll remember your face."
                self.bus.publish(
                    "sparc/tts/say", SpeakRequest(text=ack))
                self.world.conversation.append(
                    {"role": "sparc", "text": ack, "ts": time.time()})
                log.info("SPARC ENROLLED FACE: %s (%s)", name, eid)
            else:
                # outcome truth: a silent failure would let SPARC later claim success
                self.world.add_event(
                    "enroll_failed",
                    f"SPARC tried to save {name}'s face but couldn't get a clear "
                    f"enough look — it was not saved", d.entity_ids, 0.6)
                self.bus.publish("sparc/tts/say", SpeakRequest(
