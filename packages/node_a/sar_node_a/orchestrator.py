...
Run: python -m sar_node_a.orchestrator  (mosquitto must be running locally)
"""
...
log = logging.getLogger("sar.orchestrator")
...
class Orchestrator:
        ...
        for det in frame.detections:
            ...
            if frame.scene_delta == "new_track":
                ...
                if reappeared:
                    ...
                    self.bus.publish_json("sar/debug/thought", {
                        "kind": "note",
                ...
                if eid:
                    ...
                    self.world.add_event("person_left", "they left SAR's view", [eid], 0.3)
    ...
    def _drain_births(self) -> None:
        ...
        for eid, deadline in list(self._pending_births.items()):
            ...
            d = Deliberation(
                ...
                + ("" if name else " (SAR doesn't recognize them)"),
                tier=Tier.COMPETE,
    ...
    def on_sound(self, snd: SoundEvent) -> None:
        ...
        if prio >= 0.6:
            self.submit(Deliberation(
                ...
                trigger_desc=f"SAR heard what sounded like {snd.cls}",
    ...
    def run(self) -> None:
        self.bus.subscribe("sar/vision/tier0", DetectionFrame, self.on_tier0)
        self.bus.subscribe("sar/vision/rich", DetectionFrame, self.on_rich)
        self.bus.subscribe("sar/audio/transcript", Transcript, self.on_transcript)
        self.bus.subscribe("sar/audio/sound", SoundEvent, self.on_sound)
        self.bus.start()
    ...
    def execute(self, d: Deliberation, action: Action) -> None:
        ...
        if action.kind in ("say", "ask_user"):
            ...
            self.bus.publish("sar/tts/say", SpeakRequest(text=text))
            self.world.conversation.append({"role": "sar", "text": text, "ts": time.time()})
            self.world.add_event("sar_said", f'SAR said: "{text}"', d.entity_ids, 0.4)
            log.info("SAR SAYS: %s", text)
        elif action.kind == "remember":
            stmt = action.args.get("statement", "")
            self._commit_fact(stmt, source="observed")
            log.info("SAR REMEMBERS: %s", stmt)
            # a remember with no reply feels like being ignored — brief deterministic ack
            ack = "Got it — I'll remember that."
            self.bus.publish("sar/tts/say", SpeakRequest(text=ack))
            self.world.conversation.append({"role": "sar", "text": ack, "ts": time.time()})
            ...
            if eid:
                self.world.add_event("enrolled", f"SAR learned {name}'s face",
                                     [eid], 0.7)
                self._commit_fact(f"{name} is someone SAR knows by sight",
                                  source="observed", confidence=0.8)
                ack = f"Nice to meet you, {name} — I'll remember your face."
                self.bus.publish(
                    "sar/tts/say", SpeakRequest(text=ack))
                self.world.conversation.append(
                    {"role": "sar", "text": ack, "ts": time.time()})
                log.info("SAR ENROLLED FACE: %s (%s)", name, eid)
            else:
                # outcome truth: a silent failure would let SAR later claim success
                self.world.add_event(
                    "enroll_failed",
                    f"SAR tried to save {name}'s face but couldn't get a clear "
                    f"enough look — it was not saved", d.entity_ids, 0.6)
                self.bus.publish("sar/tts/say", SpeakRequest(
