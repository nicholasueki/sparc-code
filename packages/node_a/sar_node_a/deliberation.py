...
from sar_common import config, narrative
from sar_common.types import Action, new_id
...
def serialize_scene(world) -> str:
    ...
    bits = [f"It's {narrative.scene_clock()}. SAR is in the apartment, on his stand."]
    if world.present:
        people = []
        for eid, info in world.present.items():
            who = info["name"] or "someone SAR doesn't recognize"
            attend = ", looking at SAR" if info.get("attending", 0) > 0.5 else ""
        ...
        if len(unknown_ids) == 1:
            ...
            bits.append(
                "SAR has NOT saved the unrecognized person's face. "
                + ("If they tell SAR their name, SAR can save their face now."
                   if has_face
                   else "SAR cannot get a clear look at their face yet."))
        elif len(unknown_ids) > 1:
            bits.append(
                f"There are {len(unknown_ids)} people here SAR doesn't recognize; "
                "SAR can only save one new face at a time, when it's clear whose "
                "name was given.")
    else:
...
def _render_event(type_: str, description: str) -> str:
    """SAR's OWN utterances are summarized, never quoted, in the scene prose.

    Verbatim self-quotes proved to be a feedback loop: anything SAR once said
