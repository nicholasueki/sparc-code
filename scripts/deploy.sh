...
node_params() {
  case "$1" in
    a) HOST="robot-vision@robot-vision.local"; DEST="/home/robot-vision/lucas"
       REQS="pydantic paho-mqtt pyyaml httpx numpy fastapi uvicorn" ;;
    b) HOST="robot-genai@robot-genai.local"; DEST="/home/robot-genai/lucas"
       REQS="pydantic paho-mqtt pyyaml httpx fastapi uvicorn" ;;
    c) HOST="tokenator@10.1.215.33"; DEST="/Users/tokenator/lucas"
...
deploy_node() {
  ...
  if [ "$1" = "c" ]; then
    ...
    ssh "$HOST" "python3 -m venv --system-site-packages ~/lucas_venv 2>/dev/null || true; \
                 ~/lucas_venv/bin/pip install -q $REQS && echo deps-ok"
  fi
